...
次の入力は通常の学習・辞書・モデル候補を残したまま、Space変換の最初の候補ページへ表示します。日時や計算結果などの特殊候補は、次回の変換へ固定値として学習しません。

| 入力 | 候補例 |
|---|---|
| `きょう`、`あした`、`きのう`、`あさって` | 西暦、和暦、曜日 |
...
| `645ねん` | `大化元年` |
| `(10+4)*3=`、`100/8=`、`2^10=` | `42`、`12.5`、`1024` |
| `U+611B` | `愛` |
| `ばーじょん` | 実行中のKarukanバージョン |
| `かおもじ`、`にこにこ`、`しょぼーん` | 読みに対応する顔文字 |
| `あぼがど` | 訂正候補 `アボカド` |

数式は半角または全角の `=` で終え、数字、小数点、括弧、`+ - * / ^`を使用します。ゼロ除算、有限値にならない計算、構文が完結しない式、Unicodeのサロゲートまたは範囲外コードポイントは特殊候補を生成しません。

4桁時刻はGoogle日本語入力と同じく、翌日早朝を表す業務時刻表記として`0000`〜`2959`を受け付けます。`3000`以降や分が60以上の入力は時刻候補を生成しません。

ローマ字入力中は、`zh/zj/zk/zl`で`←/↓/↑/→`、`z[`と`z]`で`『`と`』`、`z,`と`z.`で`‥`と`…`、`z/`と`z-`で`・`と`〜`を直接入力できます。

配布システム辞書を読み込んでいる場合は、`よっと`から`yacht`、`あんどろいど`から`Android`のような英単語候補も選べます。既存の表記変換では、数字から全角・漢数字・大字・ローマ数字・丸数字・2進数・8進数・16進数を生成し、`はーと`や`きごう`から絵文字・記号候補を生成します。
...
1. 📝 学習キャッシュ
2. 👤 ユーザー辞書
3. 🔄 明示的な特殊変換（日付・時刻・計算・Unicode・顔文字・訂正）
4. 🤖 モデル推論
5. 📚 システム辞書（スコア順）
6. ひらがな / カタカナ
7. 🔄 表記変換（半角カタカナ・英字全角半角・記号・数値バリアント）
